...
export interface CardItem {
  ...
  title?: string;
  description?: string;
  accent?: string; // css color value for the icon bg
}
        ...
        >
          {cards.map((card, index) => {
            ...
            const cardInner = card.imgUrl ? (
              ...
              <div
                className="relative w-full h-full flex flex-col items-center justify-center gap-3 p-4 text-center select-none bg-white"
              >
                {/* Icon */}
                {card.icon && (
                  <div className="relative z-10 flex items-center justify-center w-10 h-10 rounded-xl bg-gray-100 text-gray-700 shrink-0">
                    {card.icon}
                  </div>
                )}
                {/* Title */}
                {card.title && (
                  <p className="relative z-10 font-[Archivo_Black] text-gray-950 text-xs uppercase leading-tight tracking-tight">
                    {card.title}
                  </p>
                )}
                {/* Description */}
                {card.description && (
                  <p className="relative z-10 text-gray-400 text-[10px] leading-relaxed">
                    {card.description}
                  </p>
                )}
          ...
          <div className="flex items-center gap-2">
            {cards.map((_, i) => (
              <span
                ...
                style={
                  i === centerIndex
                    ? { backgroundColor: "var(--club-blue-deep)" }
                    : {}
                }
              />
            ))}
