  ...
  return (
    <section id="departments" className="bg-[#F8FAFC] py-24 px-5 sm:px-8">
      <div className="max-w-6xl mx-auto">
        ...
        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-blue-600 mb-4">Departments</p>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {departments.map((d) => (
            ...
            >
              <span className="text-2xl text-blue-600">{d.icon}</span>
              <div>
              ...
              <div className="flex flex-wrap gap-1 mt-auto">
                {d.skills.slice(0, 3).map((s) => (
                  <span key={s} className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-blue-50 text-blue-700">
                    {s}
                  </span>
                ))}
                {d.skills.length > 3 && (
