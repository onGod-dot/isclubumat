...
const contactItems = [
  ...
  { label: "Phone", value: "020 431 9050 / 055 358 6495", href: "tel:+2330204319050", icon: Phone },
...
function ContactForm({ onSuccess }: { onSuccess?: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Message sent! You'll receive an SMS confirmation shortly.");
      setForm(emptyForm);
      onSuccess?.();
    } catch (error) {
    ...
    <form className="space-y-4" onSubmit={handleSubmit}>
      <div className="grid sm:grid-cols-2 gap-4">
        <div>
          ...
          <input type="text" required placeholder="Your name" className={inputClass} value={form.name} onChange={update("name")} />
        </div>
        <div>
          <label className="block text-xs font-semibold text-gray-500 mb-1.5">Email</label>
          <input type="email" required placeholder="your@email.com" className={inputClass} value={form.email} onChange={update("email")} />
        </div>
      </div>
      <div>
      ...
      <div>
        ...
        <input type="text" required placeholder="e.g. Partnership Enquiry" className={inputClass} value={form.subject} onChange={update("subject")} />
      </div>
      <div>
        <label className="block text-xs font-semibold text-gray-500 mb-1.5">Message</label>
        <textarea required rows={5} placeholder="Your message..." className={`${inputClass} resize-none`} value={form.message} onChange={update("message")} />
      </div>
      <button
        type="submit"
        disabled={submitting}
