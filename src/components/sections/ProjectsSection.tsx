  ...
  return (
    <section id="projects" className="bg-[#F8FAFC] py-24 px-5 sm:px-8">
      <div className="max-w-6xl mx-auto">
        ...
        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-blue-600 mb-4">Projects</p>
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-12">
          <h2 className="text-4xl sm:text-5xl font-black text-gray-950 leading-tight tracking-tight max-w-lg">
            Things we've built.
          </h2>
          <a href="#" className="text-sm font-semibold text-blue-600 hover:text-blue-800 transition whitespace-nowrap">
            View all projects →
          </a>
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {projects.map((p) => (
            <div
              key={p.title}
              className="bg-white border border-gray-100 rounded-2xl p-6 flex flex-col gap-4 hover:border-blue-200 hover:-translate-y-0.5 transition-all duration-200"
            >
              ...
              <div className="flex gap-3 text-sm font-semibold">
                <a href={p.github} className="text-gray-950 hover:text-blue-600 transition">GitHub ↗</a>
                {p.demo && <a href={p.demo} className="text-blue-600 hover:text-blue-800 transition">Live Demo ↗</a>}
              </div>
            </div>
          ))}
        </div>
